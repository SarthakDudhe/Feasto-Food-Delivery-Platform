...
const RiderMap = ({ riderCoords, customerCoords, isPickedUp }) => {
  ...
  useEffect(() => {
    ...
    // Create Coral Rider Pin
    const riderEl = document.createElement("div");
    riderEl.className = "rider-marker";
    riderEl.innerHTML = `<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M19 7c0-1.1-.9-2-2-2h-3v2h3v2.65L13.52 14H10V9H6c-2.21 0-4 1.79-4 4v3h2c0 1.66 1.34 3 3 3s3-1.34 3-3h4.18c.41 0 .8-.17 1.08-.47L19 12.35V7zM7 17c-.55 0-1-.45-1-1h2c0 .55-.45 1-1 1zm12 0c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm0-4c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1z"/></svg>`;
    ...
    destEl.innerHTML = `<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>`;
      ...
      if (geometry) {
        if (map.getSource("route")) {
          ...
          map.addLayer({
            ...
            paint: { "line-color": "#ff5a3d", "line-width": 8, "line-opacity": 0.25 },
          });
          map.addLayer({
            id: "route",
            type: "line",
            source: "route",
            layout: { "line-join": "round", "line-cap": "round" },
            paint: { "line-color": "#ff5a3d", "line-width": 4, "line-opacity": 0.95 },
          });
        }
      }
    });
  ...
  return (
    <div className="rider-map-wrapper">
      ...
      <button className="map-recenter-btn" onClick={handleRecenter} title="Recenter on My GPS">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="var(--accent-dark)">
